...
main_directory <- "ags_cascade_1.0/Synergy_results/Bootstrap/one_node/"
...
for (synergy_file in synergy_files) {
  ...
  observed_synergies_file <- file.path("ags_cascade_1.0/observed_synergies")
...
#write.table(auc_df, file = "CASCADE_one_node_boot.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
ci_auc_roc <- t.test(auc_df$auc_roc, conf.level = 0.95, mu =0.82)
print(ci_auc_roc)
...
z_score <- (auc_value - mean_auc_roc) / (sd_auc_roc/sqrt(n))
