# ULM ANALYSIS WITH TRI_GRN NETWORK FROM THE GROUP AND AGS DATA FROM CMP (https://cellmodelpassports.sanger.ac.uk/passports/SIDM00850)
...
# AGS_differential_analysis_without_Nas <- na.omit(SW_differential_analysis)

# running decopleR: ulm
results_ulm <- run_ulm(mat= SW_differential_analysis, network = TRI_df, .source='source', .target='target', .mor='sign', minsize = 5)
results_ulm

#filtering for significant p-values (using threshould of 0.05)
SW_gitsbe_df <- subset(results_ulm, p_value < 0.2)
SW_gitsbe_df
...
# removing nodes that are not in CASCADE 1.0
Park_nodes <- read_csv("node_conversion/Park_nodes.csv") %>% # importing the nodes in CASCADE 1.0 
as.data.frame()
...
# format df to training data format
  cat("Condition",file="training_files/SW_620/Park/training_SW_620_Park_ulm_TRI_.txt",sep="\n")
  cat("-",file="training_files/SW_620/Park/training_SW_620_Park_ulm_TRI_.txt",sep="\n", append = TRUE)
  cat("Response",file="training_files/SW_620/Park/training_SW_620_Park_ulm_TRI_.txt",sep="\n", append = TRUE)
  ...
  cat(node_value_string, file="training_files/SW_620/Park/training_SW_620_Park_ulm_TRI_.txt", append=TRUE)
  cat("\t", file="training_files/SW_620/Park/training_SW_620_Park_ulm_TRI_.txt", append=TRUE)
  }
  cat("\nWeight:1",file="training_files/SW_620/Park/training_SW_620_Park_ulm_TRI_.txt",sep="\n", append = TRUE)