...
results_ulm <- run_ulm(mat= AGS_differential_analysis, network = TRI_df, .source='source', .target='target', .mor='sign', minsize = 5)
results_ulm

#filtering for significant p-values (using threshould of 0.05)
AGS_gitsbe_df <- subset(results_ulm, p_value < 0.2)
AGS_gitsbe_df
