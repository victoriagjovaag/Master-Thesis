...
dorothea_df <- dorothea_hs %>%
  dplyr::filter(confidence %in% c("A", "B", "C", "D", "E", "F")) %>% #using confidence levels A, B and C
...
results_ulm <- run_ulm(mat= SW_differential_analysis, network = dorothea_df, .source='tf', .target='target', .mor='mor', minsize = 5)
results_ulm
...
# removing nodes that are not in Lu
...
# format df to training data format
  cat("Condition",file="training_files/SW_620/Lu/training_file.txt",sep="\n")
  cat("-",file="training_files/SW_620/Lu/training_file.txt",sep="\n", append = TRUE)
  cat("Response",file="training_files/SW_620/Lu/training_file.txt",sep="\n", append = TRUE)
  ...
  cat(node_value_string, file="training_files/SW_620/Lu/training_file.txt", append=TRUE)
  cat("\t", file="training_files/SW_620/Lu/training_file.txt", append=TRUE)
  }
  cat("\nWeight:1",file="training_files/SW_620/Lu/training_file.txt",sep="\n", append = TRUE)
