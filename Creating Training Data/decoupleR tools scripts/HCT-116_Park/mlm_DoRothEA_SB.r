...
# HCT_differential_analysis_without_Nas <- na.omit(HCT_differential_analysis)

# running decopleR: mlm
results_mlm <- run_mlm(mat= HCT_differential_analysis, network = dorothea_df, .source='tf', .target='target', minsize = 5)
results_mlm

#filtering for significant p-values (using threshould of 0.05)
HCT_gitsbe_df <- subset(results_mlm, p_value < 0.2)
HCT_gitsbe_df
...
Park_nodes <- read_csv("node_conversion/Park_nodes.csv") %>% # importing the nodes in Park
as.data.frame()
...
# format df to training data format
  cat("Condition",file="training_files/HCT-116/Park/training_file.txt",sep="\n")
  cat("-",file="training_files/HCT-116/Park/training_file.txt",sep="\n", append = TRUE)
  cat("Response",file="training_files/HCT-116/Park/training_file.txt",sep="\n", append = TRUE)
  ...
  cat(node_value_string, file="training_files/HCT-116/Park/training_file.txt", append=TRUE)
  cat("\t", file="training_files/HCT-116/Park/training_file.txt", append=TRUE)
  }
  cat("\nWeight:1",file="training_files/HCT-116/Park/training_file.txt",sep="\n", append = TRUE)