...
# COLO_differential_analysis_without_Nas <- na.omit(COLOdifferential_analysis)

# running decopleR: mlm
results_mlm <- run_mlm(mat= COLO_differential_analysis, network = TRI_df, .source='source', .target='target', .mor='sign', minsize = 5)
results_mlm

#filtering for significant p-values (using threshould of 0.05)
COLO_gitsbe_df <- subset(results_mlm, p_value < 0.2)
COLO_gitsbe_df
...
# converting node names to match node names in Lu network
...
# format df to training data format
  cat("Condition",file="training_files/COLO-205/Lu/training_file.txt",sep="\n")
  cat("-",file="training_files/COLO-205/Lu/training_file.txt",sep="\n", append = TRUE)
  cat("Response",file="training_files/COLO-205/Lu/training_file.txt",sep="\n", append = TRUE)
  ...
  cat(node_value_string, file="training_files/COLO-205/Lu/training_file.txt", append=TRUE)
  cat("\t", file="training_files/COLO-205/Lu/training_file.txt", append=TRUE)
  }
  cat("\nWeight:1",file="training_files/COLO-205/Lu/training_file.txt",sep="\n", append = TRUE)