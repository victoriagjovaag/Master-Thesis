...
results_ulm <- run_ulm(mat= COLO_differential_analysis, network = TRI_df, .source='source', .target='target', .mor='sign', minsize = 5)
results_ulm

#filtering for significant p-values (using threshould of 0.05)
COLO_gitsbe_df <- subset(results_ulm, p_value < 0.2)
COLO_gitsbe_df
...
# format df to training data format
  ...
  cat(node_value_string, file="training_files/COLO-205/Lu/training_filetxt", append=TRUE)
