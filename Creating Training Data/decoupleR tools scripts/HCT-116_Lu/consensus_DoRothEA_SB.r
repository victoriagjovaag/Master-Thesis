...
# finding NAs AND Infs
 HCT_differential_analysis[is.na(HCT_differential_analysis),] 

# excluding NAs from the df (if necessary)
#AGS_differential_analysis_without_Nas <- na.omit(HCT_differential_analysis)
...
HCT_gitsbe_df <- subset(results_consensus)
HCT_gitsbe_df
...
nodes_for_training_data <- converted_node_names[converted_node_names$source 
%in% filtered_nodes_list, ]
